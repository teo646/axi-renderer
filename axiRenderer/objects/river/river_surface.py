...
from axiRenderer.objects.components import Mesh
...
'''
    def draw_digital_image(self, canvas_, transformation_matrix):
        canvas_ = super().draw_digital_image(canvas_, transformation_matrix)
        if(not self.is_front_side()):
            return canvas_

        c = canvas()
        c.register_mask(ReversedMask(self.path))
        for mesh in self.world.meshes:
            c = mesh.draw_digital_image(c, transformation_matrix)
