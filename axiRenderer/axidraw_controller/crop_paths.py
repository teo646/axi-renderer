...
def crop_paths(paths, field_x_len, field_y_len):
    ...
    if((drawing_x_len/drawing_y_len-1)*(field_x_len/field_y_len-1) < 0):
        ...
        for path in paths:
            for point in path.points:
                tmp = point.coordinate[0]
                point.coordinate[0] = point.coordinate[1]
                point.coordinate[1] = tmp
    ...
    c = canvas()
    c.register_mask(ReversedMask([Point(drawing_x_center-field_x_len, drawing_y_center-field_y_len),
                                  Point(drawing_x_center+field_x_len, drawing_y_center-field_y_len),
                                  Point(drawing_x_center+field_x_len, drawing_y_center+field_y_len),
                                  Point(drawing_x_center-field_x_len, drawing_y_center+field_y_len)]))
    for path in paths:
        c.draw_path(path)
